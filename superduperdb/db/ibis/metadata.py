...
class IbisMetaDataStore(MetaDataStore):
    ...
    def get_table(self, conn, table, schema):
        # Check if the table exists
        if conn.exists_table(table):
            # If the table exists, get a reference to it
            table = conn.table(table)
        else:
            # If the table doesn't exist, create it
            table = conn.create_table(table, schema=schema)
        return table

    def get_parent_child_mappings(self):
        return self.get_table(self.conn, '_parent_child_mappings', [('parent', 'string'), ('child', 'string')] )

    def get_meta_table(self):
        return self.get_table(self.conn, '_meta', [('parent', 'string'), ('child', 'string')] )

    def get_job_table(self):
        return self.get_table(self.conn, '_objects', [('identifier', 'string'), ('time', 'timestamp'), ('status', 'string'), ('args', 'array<string>'), ('kwargs', 'string'), ('stdout', 'string'), ('stderr', 'string')] )
