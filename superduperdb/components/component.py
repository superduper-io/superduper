...
class Component(Serializable, Leaf):
    ...
    _artifacts: t.ClassVar[t.Sequence[str]] = ()
    ...
    def artifact_schema(self):
        ...
        for f in dc.fields(self):
            ...
            elif callable(getattr(self, f.name)) and not isinstance(getattr(self, f.name), Serializable):
    ...
    def encode(self, bytes_encoding: BytesEncoding | None = None, leaf_types_to_keep: t.Sequence = ()):
    ...
    @staticmethod
    def decompile(path: t.Optional[str] = None):
        ...
        for e in encodables:
            with open(os.path.join(extracted_path, e), 'rb') as f:
                ...
                references['encodable'][e] = serializers[r['_content']['datatype']].decoder(f.read())
    
        # This may get deprecated in favour of inline definitions (inside components)
        for s in references.get('serializable', {}):
            references['serializable'][s] = Serializable.decode(references['serializable'][s])


        while True:
            # extract and replace the references inside the component defitions
            for c in references['component']:
                references['component'][c] = _replace_references(references['component'][c], references)
            
            # Find the component definitions which don't refer to anything else (leaf nodes)
            identifiers = [
                c for c in references['component'] 
                if not _find_references(references['component'][c])
                and isinstance(references['component'][c], dict)
            ]

            if not identifiers:
                raise Exception('Decompile due to unspecified references {_find_references}')
            ...
            if not [c for c in references['component'] if isinstance(references['component'][c], dict)]:
                break
