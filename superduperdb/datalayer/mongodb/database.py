...
class Database(MongoDatabase, BaseDatabase):
    ...
    def _download_update(self, collection, _id, key, bytes_):
        self[collection].update_one(
            {'_id': _id},
            {'$set': {f'{key}._content.bytes': bytes_}},
            refresh=False,
        )
    ...
    def execute_query(
        self, collection, filter_=None, projection=None, **kwargs
    ):
        filter_ = filter_ or {}
    ...
    def _get_docs_from_ids(self, _ids, *query_params, features=None, raw=False):
        ...
        if query_params[1:]:
            ...
            query_params.append({})
        return list(
            self[query_params[0]].find(
                *query_params[1:], features=features, raw=raw
            )
        )
    ...
    def set_job_flag(self, identifier, kw):
        self['_jobs'].update_one(
            {'identifier': identifier}, {'$set': {kw[0]: kw[1]}}
        )
    ...
    def _unset_watcher_outputs(self, info):
        ...
        logging.info(
            f'unsetting output field _outputs.{info["key"]}.{info["model"]}'
        )
        self[collection].update_many(
            filter_,
        ...
        if watcher_info.get('target') is None:
            ...
            self[collection].bulk_write(
                [
                    UpdateOne(
                        {'_id': id},
                        {'$set': {watcher_info['target']: outputs[i]}},
                    )
                    for i, id in enumerate(_ids)
                ]
            )
