...
pickle_serializer = DataType('pickle', encoder=pickle_encode, decoder=pickle_decode, artifact=True) 
dill_serializer = DataType('dill', encoder=dill_encode, decoder=dill_decode, artifact=True)
torch_serializer = DataType('torch', encoder=torch_encode, decoder=torch_decode, artifact=True)
serializers = {'pickle': pickle_serializer, 'dill': dill_serializer, 'torch': torch_serializer}
...
class Encodable(Leaf):
    ...
    def decode(cls, r, db, reference: bool = False):
        ...
        if datatype.artifact and not datatype.reference and not reference:
            ...
            assert 'uri' in r['_content'], 'If load by reference, need a valid URI for data, found "None"'
            return Encodable(
                x=None,
                datatype=datatype,
                uri=r['_content']['uri']
