...
class MetaDataStore:
    ...
    def delete_parent_child_relationships(self, parent_component: str, parent_identifier: str):
        """
    ...
    def create_artifact_relation(self, component, identifier, uuid, artifact_ids):
        ...
        for artifact_id in artifact_ids:
            data.append({'component': component, 'identifier': identifier, 'uuid': uuid, 'artifact_id': artifact_id})

        if data:
            self.db['ArtifactRelations'].insert(data, raw=True)

    def delete_artifact_relation(self, component, identifier, artifact_ids):
        """
        ...
        for artifact_id in artifact_ids:
            self.db['ArtifactRelations'].delete(
                {'component': component, 'identifier': identifier, 'artifact_id': artifact_id}
            )
    ...
    def get_artifact_relations_for_component(self, component, identifier):
        ...
        relations = t.filter(t['component'] == component, t['identifier'] == identifier).execute()
        ids = [relation['artifact_id'] for relation in relations]
        return ids

    def get_artifact_relations_for_artifacts(self, artifact_ids):
        """
        Get all relations between an artifact and a component version.
    ...
    def delete_component(self, component: str, identifier: str):
        ...
        """
        if self.cache:
            uuids = self.db[component].distinct('uuid')
            for uuid in uuids:
                try:
    ...
    def remove_by_uuid(self, component: str, identifier: str):
        """
    ...
    def get_component_parents(self, component: str, identifier: str):
        ...
        q = t.filter(t['child_component'] == component, t['child_identifier'] == identifier).select('parent_component', 'parent_identifier')
        results = q.execute()
