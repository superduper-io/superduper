...
class Embedding(BaseOpenAI):
    ...
    def predict_one(self, text, **kwargs):
        return _Embedding.create(input=text, model=self.model_id, **kwargs)[
            'data'
        ][0]['embedding']

    @do_retry
    def _predict_a_batch(self, texts, **kwargs):
        out = _Embedding.create(input=texts, model=self.model_id, **kwargs)[
            'data'
        ]
    ...
    def predict(self, texts, batch_size=100, **kwargs):  # asyncio?
        ...
        for i in range(0, len(texts), batch_size):
            out.extend(
                self._predict_a_batch(texts[i : i + batch_size], **kwargs)
            )
        return out
