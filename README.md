...
- **Deploy Model Predictions Directly on Database:** Apply arbitary models to database without the need for additional pre-processing steps, ETL or boilerplate code.
- **Difficult Data-types**: Work on your datastore with images, video, audio and any type which can be encoded in Python.
...
- **Streaming Inference:** Continuously make predictions as new data arrive, propagating results to downstream applications in real-time. 
- **Model Chaining**: Connect multiple models to create arbitrary interdependencies for the extraction of increasingly complex features and patterns.
- **Feature Store:** No need for additional feature stores - use your datastore *as* a feature store, with arbitrary data-types supported.
- **Vector-search Database:** No need for migration to additional new-fangled vector-search databases - use your datastore as a vector-search database.
