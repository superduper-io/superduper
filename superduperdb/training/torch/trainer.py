...
class TorchTrainerConfiguration(TrainerConfiguration):
    ...
    def get_validation_dataset(
        cls, database_type, database_name, validation_set
    ):
        database = get_database_from_database_type(database_type, database_name)
        query_params = database.get_query_params_for_validation_set(
            validation_set
        )
        return QueryDataset(
            query_params, database_name, database_type, fold='valid'
        )
    ...
    def __call__(
        ...
        for k in optimizer_classes:
            optimizers.append(
                optimizer_classes[k](
                    lookup[k].parameters(), **self.optimizer_kwargs[k]
                )
            )
        ...
        return Trainer(
            ...
            saving_criterion=lambda x: self.saving_criterion(
                x, watch=self.watch
            ),
    ...
    def __call__(self):
        ...
        while True:
            for batch in self.train_dataloader:
                ...
                self.log(
                    fold='TRAIN', iteration=iteration, objective=train_objective
                )
