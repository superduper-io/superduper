...
class Collection(MongoCollection):
    ...
    def insert_many(
        self, documents, *args, verbose=True, refresh=True, **kwargs
    ):
        """
        ...
        for document in documents:
            ...
            if '_fold' not in document:
                document['_fold'] = (
                    'valid' if r < valid_probability else 'train'
                )
    ...
    def find_nearest(
        ...
        if semantic_index is not None:
            ...
            keys = si_info['keys']
            watcher = self.database._get_watcher_for_learning_task(
                semantic_index
            )
            watcher_info = self.database.get_object_info(
                watcher, variety='watcher'
            )
        else:
            watcher_info = self.database.get_object_info(
                watcher, variety='watcher'
