...
from superduperdb.components.datatype import Encodable, DataType, serializers
...
class Datalayer:
    ...
    def predict(
        ...
        **kwargs,
        ...
        out = model.predict(
            input.unpack() if isinstance(input, Document) else input,
            one=True,
            context=context.unpack() if isinstance(input, Document) else input,
            **kwargs,
        )
    ...
    def _add(
        ...
        if serialized is None:
            ...
            artifacts = [l for l in leaves if isinstance(l, Encodable)]
            children = [l for l in leaves if isinstance(l, Component)]
