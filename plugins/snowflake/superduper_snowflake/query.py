...
from superduper.base.query import Op, Query, QueryPart
...
def map_superduper_query_to_snowpark_query(session, query, primary_id: str = 'id'):
    ...
    for i, part in enumerate(query.parts):
        ...
        if part.name == 'filter':
            ...
            for i, a in enumerate(args):
                if isinstance(a, Query):
                    args[i] = map_superduper_query_to_snowpark_query(session, a, primary_id)

            for k, v in kwargs.items():
                if isinstance(v, Query):
                    kwargs[k] = map_superduper_query_to_snowpark_query(session, v, primary_id)
        ...
        if part.name == 'outputs':
            ...
            for i, table in enumerate(output_tables):
                q = q.join(table, q[f'"{primary_id}"'] == table['"_source"'], join_type="left")
            continue
