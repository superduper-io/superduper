        ...
        try:
            ...
            if db is not None:
                if isinstance(select, IbisCompoundSelect):
                    try:
                        _ = db.metadata_store.get_query(str(hash(select)))
                    except NonExistentMetadataError:
                        logging.info(f'Query {select} not found in metadata, adding...')
                        db.metadata_store.add_query(select, self.identifier)
                        logging.info('Done')
    ...
    def _predict_with_select(
        ...
        **kwargs,
        ...
        for r in tqdm.tqdm(db.execute(query)):
            ids.append(str(r[db.data_store.id_field]))
...
class Model(Component, Predictor):
    ...
    def post_create(self, db: Datalayer) -> None:
        ...
        output_component = db.data_store.create_model_table_or_collection(self)
